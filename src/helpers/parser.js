...
class JSONAPI {
    ...
    if (relationships && included) {
      Object.keys(relationships).forEach(attr => {
        ...
        if (isNil(relationship)) {
          if (enableLogging) console.warn(`JSONAPI.parse: relationship is ${relationship}`);
          ...
          relationship.forEach(relItem => {
            ...
            if (rel) {
              ...
              console.warn(`JSONAPI.parse: relationship "${relItem.type}" is provided without included`);
            }
          });
        } else {
          ...
          if (rel) {
            ...
            console.warn(`JSONAPI.parse: relationship "${relationship.type}" is provided without included`);
          }
        }
      });
    }
  ...
  parse(response) {
    ...
    if (data === undefined && errors === undefined) {
      console.warn('JSONAPI.parse: neither data nor errors provided');
    }
